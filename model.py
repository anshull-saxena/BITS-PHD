...
from config import HIDDEN, LAYERS, DROPOUT, BETA, CONF, LAMBDA
...
class DeepPIPE(nn.Module):
    ...
    def __init__(self, in_size, hidden=HIDDEN, layers=LAYERS, dropout=DROPOUT):
        super().__init__()
        self.enc = nn.LSTM(in_size, hidden, layers, batch_first=True,
                           dropout=dropout if layers > 1 else 0)
        self.dec = nn.LSTM(in_size, hidden, layers, batch_first=True,
                           dropout=dropout if layers > 1 else 0)
    ...
    def forward(self, x):
        ...
        p = self.point(z)
        return {"point": p, "lower": p - F.softplus(self.lo(z)), "upper": p + F.softplus(self.hi(z))}


def pipe_loss(o, y, beta=BETA, conf=CONF, lam=LAMBDA):
...
def interval_metrics(y, lo, hi):
    ...
    return dict(PICP=picp, MPIW=mpiw, MPIW_all=float(np.mean(hi - lo)),
                captured=int(cap.sum()), n=int(len(y)))
