"""Print result tables for paper reproduction and controller experiments."""
...
def print_tables(R):
    ...
    for k, v in R["E1_main"]["baselines"].items():
        print(
            f"{k:<22}{v['MAE']:>9.1f}{v['RMSE']:>9.1f}{v['MAPE']:>8.2f}"
            f"{v['R2']:>8.3f}{'-':>8}{'-':>8}"
        )
    print(
        f"{'DeepPIPE(two-phase)':<22}{d['MAE']:>9.1f}{d['RMSE']:>9.1f}"
    ...
    print("=" * 66)
    lam_sweep = {k: (round(v["PICP"], 3), round(v["MPIW"])) for k, v in R["E2b_lambda"].items()}
    print("lambda:", lam_sweep)
    print("beta  :", {k: (round(v["PICP"], 3), round(v["MAE"])) for k, v in R["E2c_beta"].items()})
    for k, v in R["E3_features"].items():
        print(
            f"  {k:<12}({v['n_features']:>2}f) MAE={v['MAE']:.0f} "
            f"PICP={v['PICP']:.3f} MPIW={v['MPIW']:.0f}"
        )
    ...
    for f in R["E5_rolling"]["folds"]:
        print(
            f"  fold{f['fold']} {f['test_period'][0]}..{f['test_period'][1]} "
    ...
    for k, v in R["E7_significance"]["dm_abs_and_bootstrap"].items():
        print(
            f"  vs {k:<12} DM={v['DM_abs']:>7.2f} p={v['p_abs']:.1e} | "
    ...
    for k, v in R["E9_conformal"]["levels"].items():
        print(
            f"  conformal tau={v['target']}: PICP={v['PICP']:.3f} "
            f"MPIW={v['MPIW']:.0f} kupiec_p={v['kupiec']['p']:.3f}"
        )
    for k, v in R["E6_calibration"].items():
        print(
            f"  trained  a={v['target']}: PICP={v['PICP']:.3f} "
    ...
    print(
        "  corr(width,VIX)=",
        round(R["E8_vix"]["corr_width_vix"], 3),
        "| buckets:",
        {k: round(v["mean_width"]) for k, v in R["E8_vix"]["buckets"].items()},
    )


def print_ctrl_tables(R, conf=0.90):
    print("=" * 74)
    print("DeepPIPE-Ctrl  |  base half-width q =", round(R["meta"].get("q_halfwidth", 0), 1))
    print("=" * 74)

    print(f"\nE11 — INNER CALIBRATION (target p = {conf:.2f})")
    print(f"{'policy':<20}{'PICP':>8}{'MPIW':>9}{'IntScore':>10}{'Kupiec_p':>10}{'meanM':>8}")
    for name, m in R["E11_inner"].items():
        print(
            f"{name:<20}{m['PICP']:>8.3f}{m['MPIW']:>9.0f}"
    ...
    for name, b in R["E14_backtest"].items():
        print(
            f"{name:<20}{b['ann_return']:>9.3f}{b['ann_vol']:>9.3f}"
